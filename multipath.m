...
tic
    ...
    P=[0 -0.9 -4.9 -8.0 -7.8 -23.9]; %pre calculate coefficient of filter
    h_tem=sqrt(10.^(P/10));
    Cof=zeros(1,85);
    Cof(1)=h_tem(1);
    Cof(6)=h_tem(2);
    ...
    Cof(53)=h_tem(5);
    Cof(85)=h_tem(6);
    %h_tem=sqrt(10.^(Cof/10));
    U=sqrt(sum(abs(Cof.^2)));
    h=Cof/U;
    H=fft(h,N);
    
    %the old one:
    %P=[0 -0.9 -4.9 -8 -7.8 -23.9]; %pre calculate coefficient of filter
    %h_tem=sqrt(10.^(P/10));
    %U=sum(abs(h_tem.^2));
    %h=h_tem/U;
    %H=fft(h,N);

    ZEC=1./H;    %zero-forcing equalizer coefficients.work for channel
...
semilogy(SNRdb,Pe,'c',SNRdb,Pb,'k');
